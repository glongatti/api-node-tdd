...
test('Não deve inserir uma transação sem descrição', () => {
  return request(app).post(MAIN_ROUTE)
    .set('authorization', `bearer ${user.token}`)
    .send({
      date: new Date(), ammount: 100, type: 'I', acc_id: accUser.id,
    })
    .then((res) => {
...
test('Não deve inserir uma transação sem valor', () => {
  return request(app).post(MAIN_ROUTE)
    .set('authorization', `bearer ${user.token}`)
    .send({
      description: 'Sem valor', date: new Date(), type: 'I', acc_id: accUser.id,
    })
    .then((res) => {
      expect(res.status).toBe(400);
...
test.skip('Não deve inserir uma transação sem data', () => {
  return request(app).post(MAIN_ROUTE)
    .set('authorization', `bearer ${user.token}`)
    .send({
      description: 'Sem valor', ammount: 200, type: 'I', acc_id: accUser.id,
    })
    .then((res) => {
      expect(res.status).toBe(400);
